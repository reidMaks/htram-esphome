...
`wifi`/`api`/`web_server`. Виконано (див. §5, §7, §9, §10): OTA GD32, build-id GD32 в HA,
PWM-димінг підсвітки (TIMER15), синхронізація стану LED GD32→HA + % заряду + USB/charging,
не-блокуючий бузер/мелодії (TIMER2) з **RTTTL-службами HA**. Комітиться в `main` (не пушено).

**Наступні кроки — див. розділ у кінці.** Коротко: логіка кнопки (§6) → standby (§2) →
дисплей/`DRAW_RECT` (§4, найбільший, потребує DMA-фундаменту) → доробка §8. Про 921600 —
...
## 8. 🟢 Верифікувати біти статусу батареї/заряду — частково

**Зроблено:** USB/Charging (біти 0–1) виведені окремими binary-сенсорами, а з
`Battery mV` ESP рахує **% заряду** (Li-ion крива) — усе видно в HA наживо. На
живому пристрої на USB коректно показує `USB Power=ON`, `Charging=ON`.

**Лишилось:** формально звірити семантику `USB_PRESENT`/`CHARGING` (PC13/PA15) у
всіх трьох станах (акум / USB / заряджання) і що формула `mV=(raw*3275)>>11`
тримається під навантаженням периферії (аномалія 494мВ, §6.9 — не повертається).
Тепер це легко робити прямо через сутності HA замість SRAM-проби.
...
### Крок 1 — §6 Логіка кнопки (S/L) · ✅ ВИКОНАНО
...
- В ESPHome додано `binary_sensor.button` («Button») для живого стану / нативних клік-тригерів та `text_sensor.button_action` («Button Action») з автоматичною класифікацією `single` (<0.6с) / `long` (0.6–2.8с).
- GD32 прошивку оновлено на живому пристрої через OTA.

### Крок 2 — §2 Power-off / standby · 🔴 середній, делікатний (живлення)
**Стан:** у [`main.c`](../firmware/gd32/src/main.c) вже є вхід у «standby» на утриманні ~3с, але
він робить **hard-reset MCU** (`SYSRESETREQ`) для пробудження — це не справжній standby-стан,
а перезавантаження. Треба **справжній** standby: GD32 лишається живим (always-on домен),
гасить периферію (`PF7`/`PB3` low, сенсори/буст off, LED off, дисплей → лише індикатор
батареї/заряду), і чекає кнопку для повернення в «on» **без** втрати стану.
**Ризик/пастка:** `PC15` — **НЕ** головний kill DC-DC (спроба відкочена, коміт `4dcb14c`;
деталі — [GD32_HARDWARE_MAP §6.9](GD32_HARDWARE_MAP.md)). Головна рейка самопідхоплюється;
скидання `PB3` вимикає ESP32 і він не встає сам — потрібен power-cycle. Тому чіпати рейки
живлення обережно й лише за звіреною послідовністю.
**Метод:** зчитати **еталонну заводську standby-послідовність GPIO** через SWD (пристрій у
заводському «вимкненому» стані показує індикатор батареї) — метод §6.9; повторити її.
**Файли/доки:** [`main.c`](../firmware/gd32/src/main.c), [`periph.c`](../firmware/gd32/src/periph.c),
[GD32_HARDWARE_MAP §6.6/§6.9](GD32_HARDWARE_MAP.md), [CUSTOM_FIRMWARE_SPEC §9.8](CUSTOM_FIRMWARE_SPEC.md).
**Інструмент:** `flash.py --factory` (тимчасово залити заводську й зняти GPIO standby), `tools/bench.py`.
...
### Крок 4 — §8 Верифікація батареї/заряду · 🟢 дрібний
Тепер робиться прямо в HA (є `USB Power`/`Charging`/`Battery %`). Пройти три стани
(акум / USB / заряджання), звірити біти й що формула `mV=(raw*3275)>>11` не «пливе» під
навантаженням. Опційно підправити Li-ion криву `batt_mv_to_pct` під реальні заміри.

### Наскрізні доробки (fast-follow)
- **Пресети мелодій** (RTTTL з J-A-A-M: «Ой у лузі», «Козацький марш» тощо) — як приклади/кнопки.
- **Індикація за порогами CO₂** (обговорено): HA-`number` пороги → ESP світить комбінацією LED
  (шкала G→G+Y→G+Y+R) + перемикач «LED Auto». Майже суто ESP-логіка, GD32 не чіпати.
