...
`wifi`/`api`/`web_server`. Виконано в цій ітерації: OTA GD32 (§5), build-id GD32 в HA,
PWM-димінг підсвітки (TIMER15), **синхронізація стану LED GD32→HA + % заряду + USB/charging**,
**не-блокуючий бузер/мелодії (TIMER2, `CMD_PLAY_MELODY`)**. Комітиться в `main` (не пушено).

Лишається: `DRAW_RECT`/ESP-рендер (§4), standby (§2), логіка кнопки (§6). Про 921600 —
чіп **уже на 72 МГц** (§1 знято), але для безперервного піксельного стріму вузьке
місце не baud, а RX-конвеєр GD32 (див. §4).
...
Виконано: `§1 (PLL — знято)`, `§3` (@115200), `§5 (OTA)`, `§7 (LED order)`, `§9 (wifi/api)`;
+ поза нумерацією: build-id GD32, PWM-підсвітка, sync LED-стану, % заряду, USB/charging,
не-блокуючий бузер/мелодії.

Далі: `§2 (standby)` і `§6 (button)` — незалежні, можна паралельно. `§4 (display/DRAW_RECT)` —
найбільший, і потребує фундаменту в прошивці (DMA-RX + апаратний SPI дисплея, див. §4),
тому окремим етапом. `§8` — дрібна доробка-верифікація. MQTT/окрема HA-інтеграція
**вилучені** (ESPHome native API їх замінює).
