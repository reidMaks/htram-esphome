...
### Крок 1 — §6 Логіка кнопки (S/L/XL) · 🟡 малий, самодостатній
**Стан фундаменту:** GD32 уже віддає `STATUS_FLAG_BUTTON_PRESSED` (біт 4 статусу телеметрії),
ESP уже тримає `last_status_`. Тобто «сирий» сигнал кнопки в ESP **вже є** — лишилось
на ESP додати детекцію фронтів і класифікацію тривалості.
**Зробити:**
1. У [`htram_gd32.cpp`](../esphome/custom_components/htram_gd32/htram_gd32.cpp) `loop()`/при телеметрії
   відстежувати перехід 0→1/1→0 біта кнопки з таймстемпом (`millis()`); класифікувати:
   коротке (<0.5с) / довге (~2с) / дуже довге (~10с).
2. Прокинути як події ESPHome (`binary_sensor` кнопки + `on_click`/`on_multi_click`, або
   `homeassistant.event`/тригери), щоб HA-автоматизації вішали дії.
3. Розмежування з power: **утримання ~3с для standby живе на GD32** (§2), а S/L/XL app-логіка —
   на ESP. Узгодити пороги, щоб 3с-standby не конфліктував із «довгим» на ESP.
**Нюанс точності:** телеметрія раз на 30с — надто рідко для кнопки. Треба або окремий
негайний uplink-івент кнопки з GD32 (мінідодаток до протоколу: пакет-подія при зміні біта),
або почастішати статус. **Рекомендація:** додати легкий uplink `EVT_BUTTON` (тип `0x03`)
з фронтом+тривалістю, який GD32 шле одразу при відпусканні — точніше й дешевше за полінг.
**Файли:** [`firmware/gd32/src/main.c`](../firmware/gd32/src/main.c) (детекція вже є — `btn_hold_ticks`),
[`protocol.h`](../firmware/gd32/inc/protocol.h), ESP-компонент, [§9.8](CUSTOM_FIRMWARE_SPEC.md).
