...
> (Пади: GPIO16=27, GPIO17=28 за Mischianti. `tools/swd/README.md` довго казав
> для GPIO16 «Pin 25» — виправлено 2026-09-07.)
...
> Це витяг для стенду. Повна конверсія заводського пристрою — від паяння
> до зняття RDP — у [CONVERSION.md](CONVERSION.md).
...
## Відновлення GD32, коли SWD «відповідає через раз»

**Симптом.** `pyocd` рве з'єднання на `No ACK`; іноді доходить до читання
ROM-таблиці AHB-AP і зривається там. Зниження частоти не допомагає.

**Причина.** Після обірваної заливки GD32 виконує сміття з недописаного флешу,
і випадковий код перемикає `PA13`/`PA14` з режиму SWD у GPIO. Порт живий лише
кілька мілісекунд після скидання.

**Ліки — `NRST` (нога 7 LQFP48, на платі `TP18`, див.
[GD32_HARDWARE_MAP §6.5b](GD32_HARDWARE_MAP.md)).** Штатна прошивка debugprobe
на «голому» Pico лінії скидання не виводить, тому `TP18` притискається до `GND`
вручну — пінцетом або перемичкою.

```bash
.venv/bin/python tools/swd/rescue_under_reset.py [образ]
```

1. Притисни `TP18` до `GND` і тримай.
2. Запусти скрипт — він добивається з'єднання, поки ціль у скиданні.
3. За командою `>>> ВІДПУСКАЙ ПІНЦЕТ <<<` прибери контакт. Скрипт заздалегідь
   звів `VC_CORERESET`, тож ядро зупиниться на векторі скидання, не виконавши
   жодної інструкції — і SWD лишиться живим.
4. Заливка йде **без таймауту**. Перевірено на живому залізі 2026-09-06:
   9 892 Б за 17.9 с, CRC зійшовся.

> **Ніколи не став таймаут на операцію запису у флеш.** Саме обірваний по
> таймауту OTA і призвів до цієї ситуації: GD32 лишився з недописаним образом,
> перестав піднімати `PB3`/`PF7`, і ESP знеструмилась разом із єдиним робочим
> шляхом OTA.
